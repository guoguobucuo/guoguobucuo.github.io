...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Chun🥝 %c 你正在访问Chun🥝の小家",
...
//   var start = new Date("05/02/2025 00:00:00"); // 旅行者1号开始计算的时间
//   var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
//   var unit = (dis / 149600000).toFixed(6);  // 天文单位
//   // 网站诞生时间
//   var grt = new Date("05/02/2025 00:00:00");
...
function createtime() {
  ...
  const now = new Date();
  // 网站诞生时间
  const grt = new Date("2025-02-02T00:00:00");
  const timePassed = now - grt;
  ...
  const [dnum, hnum, mnum, snum] = [days, hours, minutes, seconds].map(format);

  // 旅行者1号数据（NASA官方数据）
  const launchDate = new Date("1977-09-05T12:56:00Z"); // 发射时间
  const elapsedSeconds = (now - launchDate) / 1000;
  const speed = 17.46; // 实际平均速度约17.46 km/s
  const dis = elapsedSeconds * speed;
  const unit = (dis / 149597870.7).toFixed(6); // 精确的1天文单位值

  // 根据时间段显示不同内容
  const isDayTime = hours >= 9 && hours < 18;
  const currentTimeHtml = `
      <span style="font-size:14px;font-weight:bold">${isDayTime ? "科研摸鱼中, 什么时候能够实现财富自由呀~~" : "下班休息啦, 下班了就该开开心心地玩耍~"}</span>
      <br>
      <div style="font-size:13px;font-weight:bold">
          本站运行时长 ${dnum} 天 ${hnum} 时 ${mnum} 分 ${snum} 秒
          <i id="heartbeat" class='fas fa-heartbeat'></i><br>
          旅行者1号距地球 ${dis.toLocaleString('en-US')} km, 约为 ${unit} 天文单位 🚀
      </div>
  `;
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://i.postimg.cc/TPkB1f2k/Desktop-Background.png);
    --darkmode-bg:url(https://i.postimg.cc/TPkB1f2k/Desktop-Background.png);
    --mobileday-bg: url(https://i.postimg.cc/TPkB1f2k/Desktop-Background.png);
    --mobilenight-bg: url(https://i.postimg.cc/TPkB1f2k/Desktop-Background.png);
  }`;
}
// 切换背景主函数
function changeBg(s) {
